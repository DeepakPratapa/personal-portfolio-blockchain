...
async function main() {
    ...
    const projects = [
      {
        title: 'PayChain - Blockchain-Powered Escrow Platform with Microservices Architecture',
        description: [
          'Architected and developed a production-ready, full-stack blockchain escrow platform for the gig economy, implementing a microservices architecture with 5 independent Python FastAPI services, demonstrating advanced distributed systems design, smart contract engineering, and real-time communication patterns.',
          'Engineered a sophisticated dual refund logic smart contract system in Solidity, enabling employers to cancel jobs instantly before worker assignment or automatically refund after deadline expiration, showcasing advanced blockchain development and complex state management in decentralized applications.',
          'Implemented a centralized authentication guard system using JWT tokens with Redis-backed blacklist, MetaMask signature-based authentication eliminating password storage, and role-based access control across all microservices, demonstrating deep understanding of distributed system security and cryptographic authentication.',
          'Built advanced real-time features using WebSocket integration with React Query cache invalidation, enabling instant UI updates across components when blockchain transactions complete, job status changes, or payments are released, creating a seamless user experience with optimized state management.'
          ],
         repo: 'https://github.com/DeepakPratapa/paychain',
         caseStudy: '{"challenge":"Traditional freelance platforms rely on centralized payment processors and escrow services, creating trust issues, high fees, and potential for payment disputes. Workers fear non-payment after completing work, while employers worry about funds being locked without work completion. The challenge was to create a trustless, transparent payment system using blockchain technology while maintaining a modern, responsive user experience comparable to traditional web applications, and implementing production-ready microservices architecture with comprehensive security.","architecture":{"title":"Microservices and Blockchain Architecture","content":"The solution implements a sophisticated five-microservice architecture on the backend, combined with Ethereum smart contracts for trustless escrow, and a modern React frontend with real-time WebSocket communication. This architecture ensures scalability, maintainability, and separation of concerns while providing blockchain-powered security guarantees.","stack":[{"name":"Smart Contract Layer (Solidity 0.8.20)","desc":"PayChainEscrow contract implementing dual refund logic with cancelJob() for instant employer cancellation before worker assignment, and refundExpiredJob() for automatic refunds after deadline. Features 2% platform fee calculation, worker amount distribution, and comprehensive event logging for transparency. Deployed using Hardhat with comprehensive test coverage."},{"name":"Microservices Backend (Python FastAPI)","desc":"Five independent services: User Service (authentication, JWT, MetaMask signature verification), Job Service (CRUD, workflow, checklist management), Payment Service (blockchain integration via Web3.py), WebSocket Server (Socket.io real-time events), and API Gateway (routing, CORS). Each service implements async/await patterns with SQLAlchemy ORM for optimal performance."},{"name":"Central Auth Guard","desc":"Unified authentication module (auth_guard.py) shared across all microservices, implementing 6-layer security validation: header verification, JWT signature check, token type validation, expiration check, Redis blacklist verification, and global user token revocation. Supports role-based access (employer/worker) and service-to-service API key authentication."},{"name":"React Frontend with TanStack Query","desc":"Modern React 18 application using TanStack Query v5 for server state management with automatic cache invalidation, WebSocket integration for real-time updates, MetaMask integration via ethers.js for wallet connectivity, Tailwind CSS for responsive design, and custom hooks for WebSocket event handling and authentication flow."},{"name":"Database Layer (PostgreSQL + Redis)","desc":"PostgreSQL 16 for persistent storage with async SQLAlchemy models, JSONB columns for flexible job checklist storage, proper indexing for performance, and foreign key relationships. Redis 7 for JWT token blacklist with TTL-based expiration, pub/sub for WebSocket events, and session management."},{"name":"Real-Time Communication","desc":"WebSocket server using Socket.io with channel-based subscriptions (jobs, payments), broadcasting events (job_created, job_accepted, job_completed, payment_confirmed, checklist_updated) to connected clients, triggering React Query cache invalidation for instant UI updates without manual state management."},{"name":"Blockchain Integration","desc":"Web3.py client for Ethereum interaction, deterministic private key derivation for Ganache accounts, transaction signing and submission, event monitoring, gas estimation, and comprehensive error handling. Supports both local development (Ganache) and production deployment to Ethereum testnets/mainnet."}]},"implementation":"The project was implemented following microservices best practices with clear separation of concerns. The smart contract was developed first using Hardhat, with comprehensive unit tests for all functions including edge cases. The Payment Service acts as the blockchain gateway, translating business logic into smart contract calls. The Central Auth Guard was engineered as a shared module to ensure consistent security across all services, implementing JWT verification, token blacklist checking, and role-based authorization. The Job Service orchestrates complex workflows, deciding between cancelJob() and refundExpiredJob() based on job state and deadline. The WebSocket server broadcasts events to all connected clients, while the frontend uses React Query to automatically invalidate and refetch data when events occur, creating a real-time experience. The WalletBalance component demonstrates the power of this integration - converting from manual state management with setInterval polling to automatic updates via query invalidation. A comprehensive CI/CD approach was implemented with Docker Compose for one-command deployment, environment variable management, and network isolation for security.","scalability":"The microservices architecture enables independent scaling of each service based on load. The Job Service can scale horizontally to handle increased job creation, while the Payment Service can be optimized for blockchain transaction throughput. PostgreSQL supports read replicas for scaling read operations, and Redis cluster mode enables distributed caching. The smart contract is designed for gas optimization with efficient data structures and minimal on-chain storage. WebSocket server supports multiple instances with Redis pub/sub for message distribution. The frontend uses code splitting and lazy loading for optimal performance. Database indexes on frequently queried columns (jobs.status, jobs.employer_id, jobs.worker_id) ensure query performance at scale.","security":"Security is implemented at every layer. Smart contract features access modifiers (onlyOwner, employer validation), reentrancy protection, input validation in Solidity, and comprehensive require statements. The Central Auth Guard implements 6-layer JWT verification including signature validation, expiration checking, blacklist verification, and global user token revocation. MetaMask signature-based authentication eliminates password storage vulnerabilities. Service-to-service communication uses API keys in headers. Network isolation via Docker with internal database network prevents external access. CORS configuration restricts frontend origins. Security headers (CSP, X-Frame-Options, HSTS) protect against common attacks. Input validation using Pydantic models prevents injection attacks. SQLAlchemy ORM prevents SQL injection. Token blacklist in Redis enables immediate logout and token revocation. Wallet addresses are hashed with bcrypt before storage.","innovation":"PayChain introduces several innovative patterns: dual refund logic in smart contracts (cancel vs expire) provides flexibility while maintaining security; centralized auth guard shared across microservices eliminates code duplication while ensuring consistent security; WebSocket-triggered React Query invalidation creates real-time UI updates without complex state management; signature-based authentication with MetaMask eliminates password-related vulnerabilities; JSONB checklist storage provides flexibility while maintaining relational integrity; deterministic key derivation enables secure transaction signing in backend services; comprehensive documentation including Master Architecture document and step-by-step learning guides makes the complex system understandable and maintainable.","conclusion":"PayChain demonstrates mastery of full-stack development, blockchain integration, microservices architecture, real-time communication, and production-grade security practices. The project showcases the ability to architect complex distributed systems, implement advanced smart contract logic, design secure authentication systems, optimize frontend performance with modern state management, and create comprehensive documentation for knowledge transfer. It represents a complete, production-ready application that solves real-world trust and payment problems using cutting-edge technology while maintaining excellent user experience and code quality."}'
           },
